...
class DiffusionModule(nn.Module):
    ...
    def p_sample(self, xt, t):
        ...
        alpha_cumprod_t_prev = extract(
            self.var_scheduler.alphas_cumprod, t - 1, xt
        ) if (t > 0).all() else torch.ones_like(alpha_cumprod_t)

        # Predict noise with the network
        eps_theta = self.network(xt, t)

        # Compute the mean x_{t-1} following DDPM formula
        coef1 = 1 / alpha_t.sqrt()
        coef2 = (1 - alpha_t) / (1 - alpha_cumprod_t).sqrt()

        mean = coef1 * (xt - coef2 * eps_theta)

        # For variance sigma_t, use beta_t if t > 0, else 0 (no noise at final step)
    ...
    def compute_loss(self, x0):
        ...
        t = torch.randint(0, self.var_scheduler.num_train_timesteps, size=(batch_size,), device=x0.device).long()

        noise = torch.randn_like(x0)

        # Generate noisy samples x_t
        xt = self.q_sample(x0, t, noise)

        # Predict noise using the network
        noise_pred = self.network(xt, t)

        # Mean squared error between actual noise and predicted noise
